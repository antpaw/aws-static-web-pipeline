...
data "archive_file" "index_url_rewrite" {
  type        = "zip"
  output_path = "${path.module}/.zip/index_url_rewrite.zip"

  source {
    filename = "index.js"
    content  = file("${path.module}/lambdas/index_url_rewrite.js")
  }
}

resource "aws_lambda_function" "index_url_rewrite" {
  function_name    = "index-url-rewrite"
  filename         = data.archive_file.index_url_rewrite.output_path
...
resource "aws_iam_role" "lambda_service_role" {
  name               = "cf-s3-static-web-index-lambda-role"
  path               = "/"
...
output "qualified_arn" {
  value = aws_lambda_function.index_url_rewrite.qualified_arn
}
